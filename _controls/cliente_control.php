...
function clientes_novo(){
    ...
    unset($list);
    $r=SqlQuery("SELECT cSigla, wEnglish from paises where asia='S' order by wEnglish");
    while($l=$r->fetch(PDO::FETCH_ASSOC)){
        $list[$l['cSigla']]=$l['wEnglish'];
    }
    $msg['cCountry_asia']=$list;
    $msg['cWeb']=''; //Website -- .
...
function clientes_atualiza(){
    ...
    $msg['CompanyProfile']=$l['CompanyProfile']; //Company Business Profile -- .
    $msg['ProductInterest']=$l['ProductInterest']; //Product of Interest -- .
...
function clientes_salva(){
    ...
    $pr[':EPERSON']=filter_input(INPUT_POST, 'ePerson', FILTER_SANITIZE_STRING);
    ...
    $pr[':CEMAIL']=filter_input(INPUT_POST, 'cEmail', FILTER_VALIDATE_EMAIL);
    $pr[':CPASSW']=filter_input(INPUT_POST, 'cPassw', FILTER_SANITIZE_STRING);
    $pr[':CPASSW']=password_hash(sha1($pr[':CPASSW']), PASSWORD_DEFAULT);
    $pr[':COMPANYPROFILE']=implode(';',filter_var_array($_POST['CompanyProfile'],FILTER_SANITIZE_NUMBER_INT));
    $pr[':PRODUCTINTEREST']=implode(';',filter_var_array($_POST['ProductInterest'],FILTER_SANITIZE_NUMBER_INT));
    ...
    if($pr[':CCOMPANY']==''){
        ...
        $_SESSION['view']='home';
        $_SESSION['erro_no']=2;
        $_SESSION['erro']='<h3>Your registration was fail.</h3>';
    }
    if(filter_var($_SESSION['route'][2])==0){
        // SE 0 INSERT
        $sql="INSERT into clientes (nId ,cCompany ,cAddress ,cCity ,cCEP ,cCountry ,cWeb ,cWhatsapp ,cPhone1 ,ePerson ,cPersonalName ,cCargo ,cEmail ,CompanyProfile ,ProductInterest ,eResponsa ,eMajorSector ,nInbound ,nOutbound ,nOutboundGroup ,nOutboundAverage ,cSeller1 ,cSeller2 ,cSeller3 ,cSeller4 ,cSeller5 ,cCountry1 ,cCountry2 ,cCountry3 ,cCountry4 ,cCountry5 ,cDestino1 ,cDestino2 ,cDestino3 ,cDestino4 ,cDestino5 ,tDescription, sPassw) values ('', '".$pr[':CCOMPANY']."', '".$pr[':CADDRESS']."', '".$pr[':CCITY']."', '".$pr[':CCEP']."', '".$pr[':CCOUNTRY']."', '".$pr[':CWEB']."', '".$pr[':CWHATSAPP']."', '".$pr[':CPHONE1']."', '".$pr[':EPERSON']."', '".$pr[':CPERSONALNAME']."', '".$pr[':CCARGO']."', '".$pr[':CEMAIL']."', '".$pr[':COMPANYPROFILE']."', '".$pr[':PRODUCTINTEREST']."', '".$pr[':ERESPONSA']."', '".$pr[':EMAJORSECTOR']."', '".$pr[':NINBOUND']."', '".$pr[':NOUTBOUND']."', '".$pr[':NOUTBOUNDGROUP']."', '".$pr[':NOUTBOUNDAVERAGE']."', '".$pr[':CSELLER1']."', '".$pr[':CSELLER2']."', '".$pr[':CSELLER3']."', '".$pr[':CSELLER4']."', '".$pr[':CSELLER5']."', '".$pr[':CCOUNTRY1']."', '".$pr[':CCOUNTRY2']."', '".$pr[':CCOUNTRY3']."', '".$pr[':CCOUNTRY4']."', '".$pr[':CCOUNTRY5']."', '".$pr[':CDESTINO1']."', '".$pr[':CDESTINO2']."', '".$pr[':CDESTINO3']."', '".$pr[':CDESTINO4']."', '".$pr[':CDESTINO5']."', '".$pr[':TDESCRIPTION']."', '".$pr[':CPASSW']."')";
    } else {
        // SE >0 UPDATE
    ...
    eMail($pr[':CEMAIL'],"Sign up confirmation",'mail-thai-br.html');
    eMail("dev@thailatintrademeet.com","Sign up confirmation - ".$pr[':CEMAIL'],'mail-thai-br.html');
    return true;
}
?>